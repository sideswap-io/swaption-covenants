...
use super::setup::{claim_balance, exercise, fill_position, position_utxo};
...
fn full_exercise_pays_lender_and_closes_position(
    ...
    let before = claim_balance(&context, &fill, fill.parameters.cash_asset_id)?;
    ...
    assert_eq!(
        claim_balance(&context, &fill, fill.parameters.cash_asset_id)?,
