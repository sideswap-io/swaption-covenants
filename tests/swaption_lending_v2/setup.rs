//! Regtest setup for the Swaption position covenant.
//!
//! One default signer plays issuer + borrower (it funds the fill and holds the
//! borrower NFT); a random signer is the lender (holds the lender NFT and is
//! paid on exercise). The fill is signed by the default signer alone — the
//! covenant does not care who funds the fill, only who holds the NFTs.
...
use lending_contracts::programs::swaption_lending_v2::{
    PositionParametersV2, PositionTerms, SwaptionPositionV2, script_hash,
};
...
pub(super) struct Fill {
    pub txid: Txid,
    pub position: SwaptionPositionV2,
    pub parameters: PositionParametersV2,
...
pub(super) fn fill_position(
    ...
    let lender_script = lender.get_address().script_pubkey();

    let parameters = PositionParametersV2 {
        collateral_asset_id,
        cash_asset_id,
    ...
    let position = SwaptionPositionV2::new_filled(parameters);
...
/// Borrower (default signer) exercises `amount`, paying the lender script.
/// Returns Ok(()) once confirmed.
...
pub(super) fn lender_cash_balance(lender: &Signer, cash_asset_id: AssetId) -> anyhow::Result<u64> {
    Ok(lender
        .get_utxos_asset(cash_asset_id)?
        .iter()
