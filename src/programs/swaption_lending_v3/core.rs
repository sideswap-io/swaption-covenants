...
use crate::artifacts::swaption_lending_v3::SwaptionLendingV3Program;
use crate::programs::program::SimplexProgram;
use crate::programs::swaption_lending_v3::{PositionParametersV3, WitnessBranchV3};

/// The program takes no arguments — that is the point of v3.
#[derive(Clone)]
struct NoArguments;
...
pub struct SwaptionPositionV3 {
    program: SwaptionLendingV3Program,
    parameters: PositionParametersV3,
    remaining_debt: u64,
}
...
impl SwaptionPositionV3 {
    pub fn new_filled(parameters: PositionParametersV3) -> Self {
        Self::new(parameters, parameters.terms.buyback_amount)
    }

    pub fn new(parameters: PositionParametersV3, remaining_debt: u64) -> Self {
        assert!(remaining_debt <= parameters.terms.buyback_amount, "remaining debt can't exceed the buyback amount");
        let mut program = SwaptionLendingV3Program::new(NoArguments).with_storage_capacity(2);
    ...
    pub fn get_parameters(&self) -> &PositionParametersV3 {
        &self.parameters
    }
    ...
    pub fn attach_exercise(&mut self, ft: &mut FinalTransaction, position_utxo: UTXO, amount: u64, lender_payout_script_pubkey: Script) {
        ...
        let witness = WitnessBranchV3::Exercise {
    ...
    pub fn attach_lapse(&self, ft: &mut FinalTransaction, position_utxo: UTXO, lender_payout_script_pubkey: Script) {
        ...
        let witness = WitnessBranchV3::Lapse {
            terms: self.parameters.witness_terms(),
...
impl SimplexProgram for SwaptionPositionV3 {
    fn get_program_source_code() -> &'static str {
        SwaptionLendingV3Program::SOURCE
    }
...
pub fn tapleaf_hash_of_program() -> [u8; 32] {
    ...
    let program = SwaptionLendingV3Program::new(NoArguments);
    let cmr = simplex::simplicityhl::CompiledProgram::new(SwaptionLendingV3Program::SOURCE, Arguments::default(), true)
        .expect("v3 compiles")
        .commit()
        .cmr();
    let _ = program;
...
mod tests {
    ...
    fn params() -> PositionParametersV3 {
        let asset = |b: u8| AssetId::from_slice(&[b; 32]).unwrap();
        PositionParametersV3 {
    ...
    fn print_program_leaf() {
        println!("SWAPTION_LENDING_V3_LEAF={}", hex::encode(tapleaf_hash_of_program()));
        // A vector for the SDK's hash-only reconstruction: the test params
        // at full debt and at debt 7.
        let p = params();
        println!("V3_VECTOR_DIGEST={}", hex::encode(p.digest()));
        for debt in [3_100_000_000_000u64, 7] {
            println!("V3_VECTOR_SCRIPT_{debt}={}", hex::encode(SwaptionPositionV3::new(p, debt).get_script_pubkey().as_bytes()));
        }
    }
    ...
    fn hash_only_script_matches_simplex() {
        ...
        for debt in [3_100_000_000_000u64, 7, 0] {
            let position = SwaptionPositionV3::new(p, debt);
