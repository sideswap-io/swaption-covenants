...
use crate::artifacts::swaption_lending_v4::SwaptionLendingV4Program;
use crate::programs::program::SimplexProgram;
use crate::programs::swaption_lending_v4::{PositionParametersV4, WitnessBranchV4};
...
pub struct SwaptionPositionV4 {
    program: SwaptionLendingV4Program,
    parameters: PositionParametersV4,
    remaining_debt: u64,
}
...
impl SwaptionPositionV4 {
    pub fn new_filled(parameters: PositionParametersV4) -> Self {
        Self::new(parameters, parameters.terms.buyback_amount)
    }

    pub fn new(parameters: PositionParametersV4, remaining_debt: u64) -> Self {
        assert!(remaining_debt <= parameters.terms.buyback_amount, "remaining debt can't exceed the buyback amount");
        let mut program = SwaptionLendingV4Program::new(NoArguments).with_storage_capacity(2);
    ...
    pub fn get_parameters(&self) -> &PositionParametersV4 {
        &self.parameters
    }
    ...
    pub fn attach_exercise(&mut self, ft: &mut FinalTransaction, position_utxo: UTXO, amount: u64, lender_payout_script_pubkey: Script) {
        ...
        let witness = WitnessBranchV4::Exercise {
    ...
    pub fn attach_lapse(&self, ft: &mut FinalTransaction, position_utxo: UTXO, lender_payout_script_pubkey: Script) {
        ...
        let witness = WitnessBranchV4::Lapse {
            terms: self.parameters.witness_terms(),
    ...
    /// Last look: the position is input 0, the venue's coin at the
    /// last-look script is input 1 (the caller adds it right after), and
    /// output 0 pays the lender the whole remaining debt. The venue's own
    /// outputs (the collateral, the borrower's policy payment) follow.
    pub fn attach_last_look(&self, ft: &mut FinalTransaction, position_utxo: UTXO, lender_payout_script_pubkey: Script) {
        ...
        let position_input = PartialInput::new(position_utxo)
            .with_sequence(Sequence::ENABLE_LOCKTIME_NO_RBF)
            .with_locktime(locktime);
        let witness = WitnessBranchV4::LastLook {
            terms: self.parameters.witness_terms(),
            current_debt: self.remaining_debt,
        }
        .build_witness();
        self.add_program_input_from_partial_input(ft, position_input, witness);
        ft.add_output(PartialOutput::new(lender_payout_script_pubkey, self.remaining_debt, self.parameters.cash_asset_id));
    }
...
impl SimplexProgram for SwaptionPositionV4 {
    fn get_program_source_code() -> &'static str {
        SwaptionLendingV4Program::SOURCE
    }
...
pub fn tapleaf_hash_of_program() -> [u8; 32] {
    ...
    let program = SwaptionLendingV4Program::new(NoArguments);
    let cmr = simplex::simplicityhl::CompiledProgram::new(SwaptionLendingV4Program::SOURCE, Arguments::default(), true)
        .expect("v4 compiles")
        .commit()
        .cmr();
    let _ = program;
...
mod tests {
    ...
    fn params() -> PositionParametersV4 {
        let asset = |b: u8| AssetId::from_slice(&[b; 32]).unwrap();
        PositionParametersV4 {
    ...
    fn print_program_leaf() {
        println!("SWAPTION_LENDING_V4_LEAF={}", hex::encode(tapleaf_hash_of_program()));
        // A vector for the SDK's hash-only reconstruction: the test params
        // at full debt and at debt 7.
        let p = params();
        println!("V4_VECTOR_DIGEST={}", hex::encode(p.digest()));
        for debt in [3_100_000_000_000u64, 7] {
            println!("V4_VECTOR_SCRIPT_{debt}={}", hex::encode(SwaptionPositionV4::new(p, debt).get_script_pubkey().as_bytes()));
        }
    }
    ...
    fn hash_only_script_matches_simplex() {
        ...
        for debt in [3_100_000_000_000u64, 7, 0] {
            let position = SwaptionPositionV4::new(p, debt);
