...
use crate::artifacts::swaption_lending_v2::SwaptionLendingV2Program;
use crate::programs::program::SimplexProgram;
use crate::programs::swaption_lending_v2::{PositionParametersV2, WitnessBranchV2};

/// The program takes no arguments — that is the point of v2.
#[derive(Clone)]
struct NoArguments;
...
pub struct SwaptionPositionV2 {
    program: SwaptionLendingV2Program,
    parameters: PositionParametersV2,
    remaining_debt: u64,
}
...
impl SwaptionPositionV2 {
    pub fn new_filled(parameters: PositionParametersV2) -> Self {
        Self::new(parameters, parameters.terms.buyback_amount)
    }

    pub fn new(parameters: PositionParametersV2, remaining_debt: u64) -> Self {
        assert!(remaining_debt <= parameters.terms.buyback_amount, "remaining debt can't exceed the buyback amount");
        let mut program = SwaptionLendingV2Program::new(NoArguments).with_storage_capacity(2);
    ...
    pub fn get_parameters(&self) -> &PositionParametersV2 {
        &self.parameters
    }
    ...
    pub fn attach_exercise(&mut self, ft: &mut FinalTransaction, position_utxo: UTXO, amount: u64, lender_payout_script_pubkey: Script) {
        ...
        let witness = WitnessBranchV2::Exercise {
    ...
    /// Same contract as v1's `attach_lapse`.
    pub fn attach_lapse(&self, ft: &mut FinalTransaction, position_utxo: UTXO) {
        ...
        let witness = WitnessBranchV2::Lapse {
            terms: self.parameters.witness_terms(),
            current_debt: self.remaining_debt,
        }
        .build_witness();
        self.add_program_input_from_partial_input(ft, position_input, witness);
        ft.add_output(PartialOutput::new(Script::new_op_return(b"burn"), 1, self.parameters.lender_nft_asset_id));
    }
...
impl SimplexProgram for SwaptionPositionV2 {
    fn get_program_source_code() -> &'static str {
        SwaptionLendingV2Program::SOURCE
    }
...
pub fn tapleaf_hash_of_program() -> [u8; 32] {
    ...
    let program = SwaptionLendingV2Program::new(NoArguments);
    let cmr = simplex::simplicityhl::CompiledProgram::new(SwaptionLendingV2Program::SOURCE, Arguments::default(), true)
        .expect("v2 compiles")
        .commit()
        .cmr();
    let _ = program;
...
mod tests {
    ...
    fn params() -> PositionParametersV2 {
        let asset = |b: u8| AssetId::from_slice(&[b; 32]).unwrap();
        PositionParametersV2 {
    ...
    fn hash_only_script_matches_simplex() {
        ...
        for debt in [3_100_000_000_000u64, 7, 0] {
            let position = SwaptionPositionV2::new(p, debt);
